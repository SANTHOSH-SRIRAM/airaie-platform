...
- [ ] **Phase 10: Card Canvas (Tiptap-direct block editor)** — Replace the structured-sections Card-page (Phase 8) with a single flowing Notion-like canvas. Block editor on Tiptap directly (no BlockNote dependency — long-term ownership of the block + UI layers, ProseMirror under the hood). Typed governance blocks (Intent, Input, KPI, Method, Run, Result, Evidence, Gate, EmbedCard, EmbedRecord, AiAssist) compose freely with text/heading/list/callout/divider blocks. Slash menu + drag-drop palette. ~7 waves spanning ~24-28 dev-days. Wave 1 (10-01) shipped the framework: Tiptap install, schema, persistence, auto-migration, feature-flagged `/cards/:cardId?canvas=1` route. Wave 2 (10-02) shipped real NodeViews for Intent / Input / Result + a minimal slash menu (Intent / Input / Result, with cardinality enforcement on Intent). Wave 3 (10-03) shipped real NodeViews for KPI / Method / Run + the full slash menu (16 items: 11 typed-governance + 5 layout) with cardinality enforcement on Intent / Method / Run; `cardCanvasContext` widened to thread `card / intent / plan` from the page-level fetch. **Wave 4 (10-04) shipped real NodeViews for the last 5 typed-governance kinds (Evidence / Gate / EmbedCard / EmbedRecord + AiAssist stub) — all 11 typed-governance Tiptap nodes now mount real per-kind NodeViews.** The right-rail drag-drop palette split into Wave 10-04b (~2-3 dev-days alone). Backend `cards.body_blocks JSONB` + `PATCH /v0/cards/{id}/body` with optimistic concurrency.
...
**Plans**:
  ...
  - [ ] 10-04b — Right-rail drag-drop palette (Board Pool / Tools / Run Outputs / Records / Text Blocks / AI Assist sections) — split-off (~2-3 dev-days)
  - [ ] 10-05 — Board canvas migration with Board-specific block kinds — Wave 5 (~5-6 dev-days)
  - [ ] 10-06 — Mode-rule per-block locks + perf pass (NodeView memoization, IntersectionObserver lazy-mount) + 3-way merge UI — Wave 6 (~3 dev-days)
  - [ ] 10-07 — Remove structured Phase 8 page; remove `?legacy=1` Board fallback; canvas becomes default — Wave 7 (~2 dev-days)
