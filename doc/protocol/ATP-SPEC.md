...
**Supersedes:** [ATP-SPEC-v0.1.md](./ATP-SPEC-v0.1.md) (retained as historical reference)

---

## Changelog — v0.2 vs v0.1

v0.2 captures changes that landed in the Airaie reference runtime (Rust runner
+ Python SDK + kernel bridge) between 2026-04-23 and 2026-04-24. The changes
below are **canon**; v0.1 descriptions of the same areas are obsolete.

### Breaking changes (wire format)

- **§B.4 canonical inputs — top-level JSON array.** `inputs.json` is now a
  top-level array of `{name, value, artifact_id}` objects. The earlier
  object-wrapped form (`{"inputs": {...}}` or a flat `{name: value}` map)
  is **no longer accepted** by conformant runtimes.
- **§B.4 per-key input files.** In addition to `inputs.json`, the runtime
  MUST write a `/in/<port_name>.json` file for every parameter port, so
  simple shell tools can consume inputs without parsing the array.
- **§B.4 artifact staging path.** Artifact inputs are staged at
  `/in/<port_name>` (no extension) by the reference runner; tools MUST
  probe both `/in/<port_name>` and `/in/<port_name>.<ext>` to remain
  portable across runners that preserve extensions and runners that do not.
- **§B.5 canonical outputs — top-level JSON array.** `outputs.json` is now
  a top-level array of `{name, value?, file_path?, artifact_id?}` objects.
  The earlier `{"outputs": [...]}` wrapped form is **no longer accepted**.
- **§B.5 renamed keys.** `path` → `file_path`, `inline` → `value`. The old
  names are rejected by conformant runtimes.

### Breaking changes (semantics)

- **§5.4 `content_hash` wire format.** Airaie wire-level `content_hash`
  values MUST carry the `sha256:` algorithm prefix (e.g.
  `sha256:a1b2c3...`). ATP's internal layer MAY continue to use raw hex
  internally, but MUST prepend the prefix at the Airaie boundary. See §5.4.
- **§11.1 `governance.trust_level` enum is closed.** Accepted values are
  exactly `untested | community | tested | verified | certified`. The
  value `experimental` — which appeared in some early drafts — is
  **explicitly invalid** and MUST be rejected by conformant validators.

### Non-breaking additions

- **§B.6 tmpfs `/tmp`.** When `sandbox.filesystem: sandbox` or `readonly`
  is set (i.e. the container's root filesystem is mounted read-only), the
  runtime MUST provide a 64 MB `tmpfs` at `/tmp` inside the container.
  This addresses libc/toolchain code paths that require a writable `/tmp`
  (e.g. `tmpfile(3)` in glibc, working files in ngspice, intermediate
  output from many compilers and solvers).
- **§B.5 output `artifact_id` population.** The reference runner now
  computes SHA-256 + byte size while uploading each output file and
  attaches both to the per-port record. The Airaie control plane finalizes
  the real artifact ID (`art_xxx`). Placeholder values such as
  `uploaded:<name>` are an internal runner↔bridge convention only and MUST
  NOT be surfaced to ATP consumers.

### Document versioning vs. manifest `apiVersion`

The document version (`v0.2`) describes the **spec text**. The manifest
field `apiVersion: atp/v1` identifies the **manifest format**. These two
version tracks are independent by design:

- The manifest format evolves slowly and carries a compatibility guarantee.
  `atp/v1` is the format every conformant v0.x spec and every released SDK
  consumes. It WILL NOT change until we declare `atp/v2`.
- The spec document evolves more quickly (v0.1 → v0.2 → …) to clarify
  behaviour, add bindings, and correct wire-format mistakes without
  breaking manifest compatibility.

Consequences:

- **Manifest examples in this document keep `apiVersion: atp/v1`.** A v0.2
  spec reader consumes v1 manifests.
- **Example manifests in `atp/examples/*/manifest.yaml` are not updated.**
  They already declare `apiVersion: atp/v1` and remain correct.
- The SDK's `api_version` recognizer still accepts `atp/v1` only.
- A future `atp/v2` manifest format — when proposed — will require a
  coordinated SDK + runner + control-plane rollout and is out of scope
  for v0.2.
...
0. [Changelog — v0.2 vs v0.1](#changelog--v02-vs-v01)
...
**Wire-format rule (v0.2).** On the Airaie wire — i.e. any JSON body
crossing between the runner, the kernel bridge, and the Airaie control
plane — `content_hash` values MUST carry the algorithm prefix
(`sha256:<hex>`). The raw-hex form is REJECTED at the Airaie boundary.

Purely internal ATP layers (e.g. in-process Python structures, runner
bookkeeping) MAY continue to use raw hex strings, but MUST prepend the
`sha256:` prefix when emitting the value to any JSON contract consumed
by the control plane or by another Conformant Runtime.
...
Five trust levels form an ordered progression. The enum is **closed**:
...
Conformant validators MUST accept exactly these five values for
`governance.trust_level` and MUST reject any other value as a schema
error. In particular, the value `experimental` — which appeared in some
pre-v0.2 drafts and early community manifests — is **not valid** and was
confirmed rejected by the reference validator in 2026-04-24 conformance
testing. Tools that previously used `experimental` SHOULD migrate to
`untested` (for brand-new tools) or `community` (for tools that have
passed lint but have no empirical success data yet).
...
/tmp/atp-{job}/
├── inputs/
│   ├── inputs.json              ← TOP-LEVEL ARRAY of port records (canonical)
│   ├── <port_name>.json         ← per-parameter value (JSON-encoded)
│   ├── <artifact-port-1>        ← downloaded artifact, NO extension
│   └── <artifact-port-2>        ← downloaded artifact, NO extension
└── outputs/                     ← empty; tool writes here
```

Bind-mounted into the container as `/in` (read-only) and `/out` (read-write).

#### B.4.1 Canonical `inputs.json` shape

`inputs.json` MUST be a **top-level JSON array** where each element is an
object with exactly these three keys:

```json
[
  { "name": "mesh_file", "value": null,         "artifact_id": "/in/mesh_file" },
...
- `value` — inline JSON value for parameter/metric ports; `null` for
  artifact ports.
- `artifact_id` — path to the staged artifact file for artifact ports (see
  B.4.3); `null` for parameter/metric ports.

Conformant runners MUST emit this exact shape. Object-wrapped forms
(`{"inputs": [...]}`), flat `{name: value}` maps, and records using keys
other than `name` / `value` / `artifact_id` are **not v0.2 conformant**.

#### B.4.2 Per-key input files

In addition to `inputs.json`, the runtime MUST write one file per parameter
port:

- `/in/<port_name>.json` — contains the JSON-encoded `value` (scalar,
  object, or array). This lets a shell tool read a single port without
  parsing the full array (e.g. `THRESHOLD=$(cat /in/threshold.json)`).
- Artifact ports do NOT get a `.json` file — their payload is the raw
  artifact bytes (see B.4.3).

#### B.4.3 Artifact staging path

The reference runner stages artifact inputs at `/in/<port_name>` with **no
extension** (the port name as-is). This is a deliberate choice — it makes
the staged path stable regardless of what the original upload filename
or MIME type looked like.

However, other conformant runners MAY preserve the original extension
(e.g. `/in/mesh_file.stl`). Tools MUST therefore probe **both** locations
when resolving an artifact input:

```
1. /in/<port_name>         # reference runner (no extension)
2. /in/<port_name>.<ext>   # other runners that preserve extensions
```

Tools SHOULD fall back to reading the `artifact_id` field from
`inputs.json` if neither probe succeeds.

The `artifact_id` field in `inputs.json` MUST contain the path the runner
actually wrote to, so a tool that trusts `inputs.json` can skip the probe
entirely.

#### B.4.4 Environment variables

The runtime MUST inject:

- `ATP_JOB_ID` — the ATP job handle
- `ATP_INPUT_DIR=/in`
- `ATP_OUTPUT_DIR=/out`

> **v0.2 note (2026-04-24):** this convention matches the Airaie Rust
> runner (the reference runtime). Earlier v0.1 drafts specified
> `inputs.json` as a flat object keyed by port name or as an object
> wrapper — those forms are no longer accepted. The `/in/<port_name>`
> (no-extension) staging path was introduced in v0.2.

### B.5 Output Collection Convention

Upon container exit, the runtime collects outputs.

#### B.5.1 Canonical `outputs.json` shape

If `/out/outputs.json` exists, it MUST be a **top-level JSON array** where
...
- `file_path` — path **relative to `/out/`** for a file artifact output.
- `value` — inline JSON value for parameter/metric outputs.
- `artifact_id` — pre-existing artifact identifier, used when a tool
  uploads directly to the artifact store before exit.

Exactly one of `file_path`, `value`, or `artifact_id` SHOULD be present
per entry. Records with more than one populated are ambiguous and SHOULD
be rejected by the runner.

Conformant runners MUST reject the following v0.1-era shapes:

- Object-wrapped form `{"outputs": [...]}`
- Legacy key names `path` (now `file_path`) and `inline` (now `value`)

#### B.5.2 Auto-discovery fallback

If `/out/outputs.json` is absent, the runtime auto-discovers all files in
`/out/` and MUST match them to declared output ports by name (filename
stem matches port name) or by `artifact_kind` hint.

#### B.5.3 Upload and artifact_id population

For each `file_path` record the runner:

1. Reads the file from `/out/<file_path>`.
2. Computes **SHA-256** and **byte size** while streaming it to the artifact
   store.
3. Emits a per-port `PortValue` record carrying:
   - `name` — the declared port name
   - `content_hash` — `sha256:<hex>` (see §5.4 wire-format rule)
   - `size_bytes` — the byte count
   - A runner-local placeholder `artifact_id` (see below)
4. Hands the `PortValue` list to the Airaie control plane, which assigns
   the final canonical `art_xxx` identifier and rewrites `artifact_id` in
   the result.

**Placeholder convention (internal).** Between the runner and the kernel
bridge, the runner uses the placeholder `uploaded:<port_name>` as a
temporary `artifact_id`. This placeholder is an **internal implementation
detail** and MUST NOT be surfaced to API consumers — by the time a
`JobResult` or `artifact` event is emitted to an ATP consumer, every
`artifact_id` MUST be either:

- a final `art_xxx` identifier assigned by the control plane, or
- a pre-existing artifact identifier provided by the tool via
  `outputs.json`.

Metric and parameter outputs (`value` records) are read inline and
populate `JobResult.metrics` / `JobResult.outputs` without any artifact
store interaction.

> **v0.2 note (2026-04-24):** v0.1 drafts used `path` / `inline` key names
> and an object-wrapped outer form. Those forms are no longer accepted.
> The runner-side `sha256 + size` computation during upload, and the
> `uploaded:<name>` placeholder convention, were introduced in v0.2.
...
| `sandbox.filesystem: sandbox` | `--read-only` + `--tmpfs /tmp:size=64m` + writable `/out` only |
| `sandbox.filesystem: readonly` | `--read-only` + `--tmpfs /tmp:size=64m` |
...
#### B.6.1 Writable `/tmp` tmpfs (v0.2)

When the runner applies a read-only root filesystem (i.e. the underlying
container runtime is invoked with `readonly_rootfs: true`, which occurs
for both `sandbox.filesystem: sandbox` and `sandbox.filesystem: readonly`),
the runtime MUST mount a **64 MB `tmpfs` at `/tmp`** inside the container.

Rationale: many trusted binaries assume `/tmp` is writable and fail on a
fully read-only root. Empirically affected code paths include:

- **glibc** — `tmpfile(3)`, some locale initialization paths
- **ngspice** — intermediate simulation files
- **LaTeX, LibreOffice, many Fortran solvers** — scratch output
- **Python** — `tempfile.mkstemp` with default `TMPDIR`

64 MB is a pragmatic default that fits all tested OSS solvers without
materially enlarging the container's memory footprint. Tools that need
more scratch space SHOULD write to `/out` (which is always writable)
rather than to `/tmp`.

Runners that implement a different container runtime MAY choose an
equivalent mechanism (e.g. an overlay ramdisk) provided `/tmp` is
writable inside the sandbox and its contents do not persist across
invocations.
...
- **v0.2 (2026-04-24)** — Canonicalized `inputs.json` / `outputs.json` as
  top-level arrays of `{name, value, artifact_id}` (inputs) and
  `{name, value?, file_path?, artifact_id?}` (outputs). Runners stage
  artifact inputs at `/in/<port_name>` (no extension); tools must probe
  both extensionless and extension-preserving paths. Required `sha256:`
  prefix on Airaie-wire `content_hash` values. Mandated 64 MB tmpfs at
  `/tmp` when the root filesystem is read-only. Closed `governance.trust_level`
  enum to exactly five values; `experimental` is explicitly rejected.
  Documented runner SHA-256 + size computation during output upload and
  internal-only `uploaded:<name>` placeholder. Clarified that
  `apiVersion: atp/v1` (manifest format) is independent of the spec
  document version. See [CHANGELOG section at top of document](#changelog--v02-vs-v01)
  for the full list.
...
|---|---|---|
| v0.3 | +4 weeks | CLI, HTTP, MCP binding specifications |
| v0.4 | +6 weeks | Python, Jupyter, hardware, human-task, workflow binding specs |
