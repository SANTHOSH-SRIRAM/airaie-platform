    ...
    <div className="py-3 border-b border-[#f0f0ec] last:border-b-0 cursor-pointer hover:bg-[#f8f8f7] transition-colors duration-100 rounded" onClick={onClick}>
      <div className="flex items-center justify-between mb-1">
        <div className="flex items-center gap-2 min-w-0">
          <CircleDot size={14} className={cn('shrink-0', isWaiting ? 'text-[#ff9800]' : 'text-[#2196f3]')} />
        ...
        <div className="flex items-center gap-2 shrink-0 ml-2">
          {!isWaiting && (
            <div className="w-[80px] h-[4px] bg-[#e8e8e8] rounded-full overflow-hidden">
              <div
                className="h-full bg-[#2196f3] rounded-full transition-[width] duration-300"
                style={{ width: `${progress}%` }}
              />
            </div>
          )}
          <span className="text-[13px] font-mono text-[#4caf50]">${run.costUsd.toFixed(2)}</span>
        </div>
      </div>
      <div className={cn('flex items-center justify-between', isWaiting ? 'text-[#ff9800]' : 'text-[#949494]')}>
        ...
        {isWaiting && (
          <span className="ml-2 bg-[#fff3e0] text-[#ff9800] text-[11px] font-medium px-[8px] py-[4px] rounded-[6px]">
            Needs review
          </span>
        )}
      </div>
    ...
    <div className="bg-white rounded-[12px] border border-[#ece9e3] shadow-[0px_1px_8px_0px_rgba(0,0,0,0.05)] p-[20px]">
      <div className="flex items-center justify-between pb-[16px]">
        <div className="flex items-center gap-2">
          <Activity size={14} className="text-[#2196f3]" />
