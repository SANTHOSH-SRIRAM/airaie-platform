import {
  GitBranch, ExternalLink, Plus, RotateCcw, FileInput, LayoutGrid, Play,
  MoreHorizontal, BookOpen, Link2, Layers, ArrowRight,
} from 'lucide-react';
...
const recentWorkflows = [
  { name: 'Data_Pipeline_ETL', status: 'success' as const, detail: 'Completed successfully', time: '25m ago' },
  { name: 'Model_Training_V3', status: 'warning' as const, detail: 'Awaiting approval', time: '2h ago' },
  { name: 'Deploy_Staging_CI', status: 'success' as const, detail: 'All checks passed', time: '4h ago' },
  { name: 'Batch_Export_CSV', status: 'danger' as const, detail: 'Timeout on step 4', time: '1d ago' },
];
...
const stats = { running: 3, completed: 47, failed: 2, total: 52 };
  ...
  return (
    <div className="p-6 space-y-6 max-w-[1584px]">
      ...
      <div className="flex items-start justify-between">
        <div>
          <h1 className="text-[2rem] font-light text-content-primary leading-tight tracking-tight">Workflows</h1>
          <p className="text-sm text-content-helper mt-1.5">Create, monitor, and manage automation workflows across your platform.</p>
        </div>
        <div className="flex items-start gap-4 shrink-0">
          ...
          <Button variant="tertiary" size="md" iconRight={<ExternalLink size={14} />} onClick={openStudio}>Open Workflow Studio</Button>
        </div>
      </div>
        ...
        <div className="flex">
          <div className="flex-1 px-5 py-6 border-r border-card-border-inner">
            <div className="flex items-center gap-2 mb-3">
              ...
              <h2 className="text-sm font-semibold text-content-primary uppercase tracking-wide">Start Workflow</h2>
            </div>
            <p className="text-sm text-content-secondary leading-relaxed mb-6">Design new automation pipelines, configure triggers, and define execution steps.</p>
            <div className="flex items-center gap-3">
              <Button variant="primary" size="md" icon={<Plus size={16} />} onClick={openStudio}>NEW WORKFLOW</Button>
              <Button variant="tertiary" size="md" icon={<RotateCcw size={14} />}>OPEN RECENT</Button>
              <span className="text-xs text-content-placeholder font-mono ml-1">#WF_STUDIO_V1</span>
            </div>
          </div>
      ...
      <div className="grid grid-cols-3 gap-4">
        ...
        <Card variant="elevated" className="flex-1">
          <div className="flex items-center justify-between px-5 h-12 border-b border-card-border-inner bg-card-header">
            <h2 className="text-xs font-semibold text-content-primary uppercase tracking-wider">Workflow Stats</h2>
            <Badge variant="success" badgeStyle="outline">HEALTHY</Badge>
          </div>
          <div className="px-5 py-5 space-y-5">
            <ProgressBar label="Success Rate" value={Math.round((stats.completed / stats.total) * 100)} variant="blue" striped />
            <ProgressBar label="Active Capacity" value={Math.round((stats.running / 10) * 100)} variant="blue" striped />
